...
trait Router[BasePage] extends Router.In[BasePage] with Router.Out[BasePage]
object Router {
  /**
  ...
  class Object[BasePage](
    ...
    override def pageForAbsoluteUrl(url: String): Option[BasePage] = {
    ...
    override def pageForRelativeUrl(url: String): Option[BasePage] = {
    ...
    override def absoluteUrlForPage(page: BasePage): String = {
      origin + relativeUrlForPage(page)
    }

    override def relativeUrlForPage(page: BasePage): String = {
    ...
    override def replaceState(page: BasePage): Unit = {
      routeEventBus.writer.onTry(Try(pageToRouteEvent(page, replace = true, fromPopState = false)))
    }

    override def replacePageTitle(title: String): Unit = {
      dom.document.title = title
    }

    override def forcePage(page: BasePage): Unit = {
